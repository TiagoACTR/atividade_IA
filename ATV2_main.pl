...
lugar('A Universidade Federal do Ceará campus Jardins de Anita, fica localizado a 137 quilômetros de Fortaleza').
...
quantidade_de_cursos('3').
cursos('Segurança da Informação, Ciência de Dados, Análise e Desenvolvimentos de Sistemas').
...
quantidade_de_cursos(X, Y) :-
    nome(X),
    quantidade_de_cursos(Y).
